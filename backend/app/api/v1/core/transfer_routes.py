...
from app.schemas.transfer import TransferConfirmPinRequest, TransferConfirmRequest, TransferInitiateRequest, TransferRequestOtpRequest
...
async def request_transfer_otp(
    request: Request,
    payload: TransferRequestOtpRequest,
    auth: AuthContext = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    service = TransferService()
    await service.request_transfer_otp(
        db,
        user=auth.user,
        transfer_id=payload.transfer_id,
    )
